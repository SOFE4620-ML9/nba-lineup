...
nix-shell --pure --argstr srcDir $(pwd) -p "python312.withPackages(ps: with ps; [ pandas ])" --run "python -c \"import pandas as pd; df = pd.read_csv('output/predictions.csv'); print(df['Fifth_Player'].value_counts())\""
